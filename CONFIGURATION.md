...
**Обход локального хука** (`git commit --no-verify`) технически возможен. Обратите внимание, что текущий CI-пайплайн GitHub Actions запускается **после** завершения операции push и обновления веток на сервере. Сбой CI-пайплайна заблокирует последующий Merge pull-request'а, но **не предотвратит** попадание утекшего секрета на сервер GitHub, если локальный хук был обойдён. 

Для гарантированной блокировки небезопасных пушей на стороне сервера необходимо включить встроенную функцию GitHub: **Repository Settings -> Code security and analysis -> Secret scanning -> Push protection**.
